...
- (void)requestDidLoadStreamableURL:(DBRequest *)request {
    if (request.error) {
        ...
        const char * expiresStr = [(NSString *)[response objectForKey:@"expires"] UTF8String];
        struct tm time; strptime(expiresStr, "%a, %d %b %Y %H:%M:%S %z", &time);
        NSDate * expireDate = [NSDate dateWithTimeIntervalSince1970:mktime(&time)];
        
        if ([delegate respondsToSelector:@selector(restClient:loadedStreamableURL:thatExpires:forFile:)])
            [delegate restClient:self loadedStreamableURL:url thatExpires:expireDate forFile:path];
